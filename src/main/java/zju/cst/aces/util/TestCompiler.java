...
public class TestCompiler {
    ...
    public TestCompiler(Path testOutputPath, Path compileOutputPath, Path targetPath, List<String> classpathElements, List<String> dependList) {
        this.code = "";
        this.testOutputFolder = testOutputPath.toFile();
        this.buildFolder = compileOutputPath.toFile();
        this.buildBackupFolder = targetPath.resolve("test-classes-backup").toFile();
        this.targetTestsFolder = targetPath.resolve("test-classes").toFile();
        this.classpathElements = classpathElements;
        this.dependencyPaths = dependList;
    }
    
    public TestCompiler(String code, Path testOutputPath, Path compileOutputPath, Path targetPath, List<String> classpathElements, List<String> dependList) {
        this.code = code;
        this.testOutputFolder = testOutputPath.toFile();
        this.buildFolder = compileOutputPath.toFile();
        this.buildBackupFolder = targetPath.resolve("test-classes-backup").toFile();
...
//            urls.add(targetTestsFolder.toURI().toURL());
    ...
    public boolean compileTest(final String sourceCode, String className, Path outputPath, PromptInfo promptInfo) {
        ...
        try {
            ...
            // System.out.println("CALSS PATHS");
            ...
            Iterable<String> options = Arrays.asList("-classpath", String.join(this.OS.contains("win") ? ";" : ":", tmpElements),
                    "-d", buildFolder.toPath().toString());
    ...
    public void exportError(List<String> errors, Path outputPath) {
        exportError(this.code, errors, outputPath);
    }

    public void exportError(String sourceCode, List<String> errors, Path outputPath) {
        try {
