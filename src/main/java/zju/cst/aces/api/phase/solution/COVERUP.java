        ...
        if (runTest(config, pc.getFullTestName(), promptInfo, promptInfo.getRound())) {
            try {
                ...
                if (lineCoverage == 100) {
                    ...
                    if (!exportValidatedTest(config, code, savePath, pc.getFullTestName(), promptInfo)) {
                        return false;
                    }
                ...
                if (promptInfo.coverage_improve_time >= config.max_coverage_improve_time) {
                    if (!exportValidatedTest(config, promptInfo.max_coverage_test_code, savePath, pc.getFullTestName(), promptInfo)) {
                        return false;
                    }
                    config.markExportedTest();
                    return true;
