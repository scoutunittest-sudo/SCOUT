...
public class CodeCoverageAnalyzer {
    ...
    // 修改此行代码，传入当前上下文类加载器作为父类
        ...
        Map<String, Object> result = getMethodCoverageInfo(coverageBuilder, targetClassName, methodSignature, targetClassSourceDir);
        result.put("testsFoundCount", summary.getTestsFoundCount());
        result.put("testsSucceededCount", summary.getTestsSucceededCount());
        result.put("testsFailedCount", summary.getTestsFailedCount());
        result.put("testsAbortedCount", summary.getTestsAbortedCount());
        result.put("failureMessages", failureMessages(summary));
        return result;
    }
    ...
    private byte[] compileAndInstrument(String sourceCode, String className, Instrumenter instr, String targetClassCompiledDir, List<String> dependencies) throws IOException {
        ...
        if (success == null || !success.booleanValue()) {
            throw new IOException("Compilation failed for " + className + ": " + diagnostics(diagnostics));
        }
    ...
    private List<String> failureMessages(TestExecutionSummary summary) {
        ...
        summary.getFailures().forEach(failure -> {
            ...
            messages.add("Error in " + testName + ": " + (exception == null ? "unknown failure" : exception.toString()));
        });
        return messages;
    }

    private String diagnostics(DiagnosticCollector<JavaFileObject> diagnostics) {
        if (diagnostics == null || diagnostics.getDiagnostics().isEmpty()) {
            return "no diagnostics";
        }
        List<String> messages = new ArrayList<>();
        for (Diagnostic<? extends JavaFileObject> diagnostic : diagnostics.getDiagnostics()) {
            messages.add("line " + diagnostic.getLineNumber() + ": " + diagnostic.getMessage(Locale.ROOT));
        }
