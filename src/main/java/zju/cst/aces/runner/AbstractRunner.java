...
public abstract class AbstractRunner {
    ...
    private static final int SCOUT_ERROR_SALVAGE_MAX_PASSES = 3;
    ...
    public static boolean exportValidatedTest(Config config, String code, Path savePath,
                                              String fullTestName, PromptInfo promptInfo) {
        if (code == null || code.trim().isEmpty()) {
            if (config != null && config.getLogger() != null) {
                config.getLogger().warn("Skip exporting empty generated test < "
                        + simpleTestName(fullTestName, savePath) + " >.");
            }
            return false;
        }
        if (!finalCompileBeforeExport(config, code, savePath, fullTestName, promptInfo)) {
            return false;
        }
        ...
        try {
            ...
            recordTiming(config, "export", startNanos);
        }
        return true;
    }

    private static boolean finalCompileBeforeExport(Config config, String code, Path savePath,
                                                    String fullTestName, PromptInfo promptInfo) {
        if (config == null || config.getValidator() == null) {
            return true;
        }
        String testName = simpleTestName(fullTestName, savePath);
        Path diagnosticsPath = finalCompilationErrorPath(config, testName);
        boolean valid = semanticValidate(config, code, testName, diagnosticsPath, promptInfo);
        if (!valid) {
            config.markCompilationError();
            if (config.getLogger() != null) {
                config.getLogger().warn("Skip exporting non-compilable generated test < "
                        + testName + " >. Diagnostics: " + diagnosticsPath);
            }
        }
        return valid;
    }

    private static String simpleTestName(String fullTestName, Path savePath) {
        if (fullTestName != null && !fullTestName.trim().isEmpty()) {
            String trimmed = fullTestName.trim();
    ...
    private static Path finalCompilationErrorPath(Config config, String testName) {
        Path errorOutput = config.getErrorOutput();
        if (errorOutput == null) {
            errorOutput = config.getTmpOutput();
        }
        if (errorOutput == null) {
            errorOutput = Paths.get(System.getProperty("java.io.tmpdir"), "chatunitest-errors");
        }
        return errorOutput.resolve(testName + "_FinalCompilationError.txt");
    }
...
//        }
    ...
    public static boolean runTest(Config config, String fullTestName, PromptInfo promptInfo, int rounds) {
        ...
        if (config != null) {
            String statusClassName = promptInfo.getFullClassName();
            if (statusClassName == null || statusClassName.trim().isEmpty()) {
                statusClassName = fullTestName;
            }
            config.updateCurrentStatusMethod(statusClassName, promptInfo.getMethodInfo());
        }
        ...
        // Compilation
        updateStatusStep(config, "Compiling");
        ...
        boolean compileResult = semanticValidate(config, code, testName, compilationErrorPath, promptInfo);
        if (!compileResult) {
            config.markCompilationError();
            if (trySalvageScoutErrorTests(config, fullTestName, testName, promptInfo, testProcessor, code,
                    compilationErrorPath, executionErrorPath, rounds, null, null)) {
                return true;
            }
        ...
        if (config.isNoExecution()) {
            if (!isScoutPhase(config)) {
                if (!exportGeneratedTest(config, code, savePath, fullTestName, promptInfo)) {
                    return false;
                }
            }
        ...
        // Execution
        updateStatusStep(config, "Running");
        TestExecutionSummary summary = executeTest(config, fullTestName);
        if(summary==null){
            config.markTimeout();
        ...
        if (summary.getTestsFailedCount() > 0 || summary.getTestsSucceededCount() == 0) {
            ...
            errors = ensureExecutionErrors(summary, errors);
            if (isScoutPhase(config)) {
                if (trySalvageScoutErrorTests(config, fullTestName, testName, promptInfo, testProcessor, code,
                        compilationErrorPath, executionErrorPath, rounds, summary, errors)) {
                    return true;
                }
                return false;
            }
            ...
            if (testProcessed != null) {
                ...
                if (semanticValidate(config, testProcessed, testName, compilationErrorPath, null)) {
                    if (runtimeValidate(config, fullTestName)) {
                        promptInfo.setUnitTest(testProcessed);
                        if (!exportGeneratedTest(config, testProcessed, savePath, fullTestName, promptInfo)) {
                            return false;
                        }
            ...
            recordExecutionFailure(config, promptInfo, code, errors, executionErrorPath, rounds);
            return false;
        }
        if (!config.isStatusOnlyOutput()) {
            summary.printTo(new PrintWriter(System.out));
        }
        if(!config.phaseType.equals("COVERUP") && !isScoutPhase(config)){
            if (!exportGeneratedTest(config, code, savePath, fullTestName, promptInfo)) {
                return false;
            }
        }
    ...
    private static boolean exportGeneratedTest(Config config, String code, Path savePath,
                                               String fullTestName, PromptInfo promptInfo) {
        updateStatusStep(config, "Exporting");
        if (!exportValidatedTest(config, code, savePath, fullTestName, promptInfo)) {
            return false;
        }
        if (config != null) {
            config.markExportedTest();
            markMethodWithValidTest(config, fullTestName, promptInfo);
        }
        return true;
    }

    private static boolean semanticValidate(Config config, String code, String testName,
                                            Path diagnosticsPath, PromptInfo promptInfo) {
        long startNanos = System.nanoTime();
        Config.ResourceLease lease = acquireResource(config, "compile");
        try {
            return config.getValidator().semanticValidate(code, testName, diagnosticsPath, promptInfo);
        } finally {
            releaseResource(lease);
            recordTiming(config, "compile", startNanos);
        }
    }

    private static TestExecutionSummary executeTest(Config config, String fullTestName) {
        long startNanos = System.nanoTime();
        Config.ResourceLease lease = acquireResource(config, "run");
        try {
            return config.getValidator().execute(fullTestName);
        } finally {
            releaseResource(lease);
            recordTiming(config, "run", startNanos);
        }
    }

    private static boolean runtimeValidate(Config config, String fullTestName) {
        long startNanos = System.nanoTime();
        Config.ResourceLease lease = acquireResource(config, "run");
        try {
            return config.getValidator().runtimeValidate(fullTestName);
        } finally {
            releaseResource(lease);
            recordTiming(config, "run", startNanos);
        }
    ...
    private static Config.ResourceLease acquireResource(Config config, String category) {
        return config == null ? null : config.acquireResource(category);
    }

    private static void releaseResource(Config.ResourceLease lease) {
        if (lease != null) {
            lease.close();
        }
    }

    private static void recordTiming(Config config, String category, long startNanos) {
        if (config != null) {
            config.recordTimingSince(category, startNanos);
        }
    }

    private static void updateStatusStep(Config config, String step) {
        if (config != null) {
            config.setCurrentStatusStep(step);
        }
    }

    private static boolean isScoutPhase(Config config) {
        return config != null && "SCOUT".equals(config.phaseType);
    }

    private static boolean trySalvageScoutErrorTests(Config config,
                                                     String fullTestName,
                                                     String testName,
                                                     PromptInfo promptInfo,
                                                     TestProcessor testProcessor,
                                                     String code,
                                                     Path compilationErrorPath,
                                                     Path executionErrorPath,
                                                     int rounds,
                                                     TestExecutionSummary failedSummary,
        ...
        TestExecutionSummary currentSummary = failedSummary;
        List<String> currentRuntimeErrors = runtimeErrors;

        for (int pass = 1; pass <= SCOUT_ERROR_SALVAGE_MAX_PASSES; pass++) {
            updateStatusStep(config, "Repairing");
            promptInfo.setUnitTest(currentCode);
            String testProcessed = currentSummary == null
                    ? testProcessor.removeErrorTestMethods(promptInfo, promptInfo.getErrorMsg())
                    : testProcessor.removeErrorTestMethods(promptInfo, currentSummary);
            if (testProcessed == null || testProcessed.equals(currentCode)) {
                break;
            }

            config.getLogger().debug("SCOUT salvage pass " + pass + " removed method-local failing test(s).");
            currentCode = testProcessed;
            promptInfo.setUnitTest(currentCode);

            updateStatusStep(config, "Compiling");
            boolean compileResult = semanticValidate(config, currentCode, testName, compilationErrorPath, promptInfo);
            if (!compileResult) {
                config.markCompilationError();
                currentSummary = null;
                currentRuntimeErrors = null;
                continue;
            }
            if (config.isNoExecution()) {
                if (!isScoutPhase(config)) {
                    if (!exportGeneratedTest(config, currentCode, promptInfo.getTestPath(), fullTestName, promptInfo)) {
                        return false;
                    }
                }
                config.getLogger().info("Processed test for method < " + promptInfo.getMethodInfo().getMethodName() + " > generated successfully round " + rounds);
            ...
            updateStatusStep(config, "Running");
            currentSummary = executeTest(config, fullTestName);
            if (currentSummary == null) {
                config.markTimeout();
                config.getLogger().warn("Test for method < " + promptInfo.getMethodInfo().getMethodName() + " > execution timeout round ");
                return false;
            }
            currentRuntimeErrors = extractErrorBySummary(currentSummary, fullTestName);
            if (currentSummary.getTestsFailedCount() == 0 && currentSummary.getTestsSucceededCount() > 0) {
                if (!config.isStatusOnlyOutput()) {
                    currentSummary.printTo(new PrintWriter(System.out));
                }
                if (!isScoutPhase(config)) {
                    if (!exportGeneratedTest(config, currentCode, promptInfo.getTestPath(), fullTestName, promptInfo)) {
                        return false;
            ...
            config.markRuntimeError();
            currentRuntimeErrors = ensureExecutionErrors(currentSummary, currentRuntimeErrors);
        }

        promptInfo.setUnitTest(currentCode);
        if (currentSummary != null) {
            recordExecutionFailure(config, promptInfo, currentCode,
                    ensureExecutionErrors(currentSummary, currentRuntimeErrors), executionErrorPath, rounds);
        } else if (promptInfo.getErrorMsg() != null
                && promptInfo.getErrorMsg().getErrorType() == TestMessage.ErrorType.COMPILE_ERROR) {
            config.getLogger().info("Test for method < " + promptInfo.getMethodInfo().getMethodName() + " > compilation failed round " + rounds);
        }
        return false;
    }

    private static void markMethodWithValidTest(Config config, String fallbackClassName, PromptInfo promptInfo) {
        if (config == null || promptInfo == null) {
            return;
        }
        String fullClassName = promptInfo.getFullClassName();
        if (fullClassName == null || fullClassName.trim().isEmpty()) {
            fullClassName = fallbackClassName;
        }
        if (promptInfo.getMethodInfo() != null) {
            config.markMethodWithValidTest(fullClassName, promptInfo.getMethodInfo());
            return;
        }
        config.markMethodWithValidTest(fullClassName, promptInfo.getMethodSignature(), promptInfo.getMethodName());
    }

    private static void recordExecutionFailure(Config config, PromptInfo promptInfo, String code,
                                               List<String> errors, Path executionErrorPath, int rounds) {
    ...
    private static List<String> ensureExecutionErrors(TestExecutionSummary summary, List<String> errors) {
        if (errors != null && !errors.isEmpty()) {
            return errors;
        }
        List<String> fallback = new ArrayList<>();
        fallback.add(summary.getTestsFailedCount() + " test(s) failed, "
                + summary.getTestsSucceededCount() + " test(s) succeeded, "
                + summary.getTestsAbortedCount() + " test(s) aborted.");
        return fallback;
    }
        ...
        summary.getFailures().forEach(failure -> {
            for (StackTraceElement st : failure.getException().getStackTrace()) {
                if (st.getClassName().contains(matchName)) {
                    String testName = failure.getTestIdentifier() == null
                            ? "unknown test"
                            : failure.getTestIdentifier().getLegacyReportingName();
