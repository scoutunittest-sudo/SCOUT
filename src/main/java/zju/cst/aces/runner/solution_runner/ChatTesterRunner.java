...
public class ChatTesterRunner extends MethodRunner {
    ...
    public ChatTesterRunner(Config config, String fullClassName, MethodInfo methodInfo, int timeout, long startTime) throws IOException {
        super(config, fullClassName, methodInfo, timeout, startTime);
    ...
    public boolean startRounds(final int num) throws IOException {
        ...
        config.setValidator(new TesterValidator(config.getTestOutput(), config.getCompileOutputPath(),
                config.getProject().getBasedir().toPath().resolve("target"),
                config.getClassPaths(), config.getDependencyPaths())); // 设置chattester的专属验证器
