...
public class MUTAPRunner extends MethodRunner {
    ...
    public MUTAPRunner(Config config, String fullClassName, MethodInfo methodInfo, int timeout, long startTime) throws IOException {
    ...
    public boolean startRounds(final int num) throws IOException {
        ...
        if (this.timeout != -1 && elapsed / 1000 >= this.timeout) {
            System.exit(1);
        }
    ...
    private boolean runPitMutationTests() {
        try {
            ...
            if (config.isStatusOnlyOutput()) {
                processBuilder.redirectOutput(discardRedirect());
                processBuilder.redirectError(discardRedirect());
            } else {
